...
### Requirement: Reasoning summary emission defaults to enabled
By default, the agent SHALL emit a reasoning summary as a system-visible transcript entry when available.

#### Scenario: Summary is emitted as a system message
- **GIVEN** reasoning exposure is configured as `summary` (default)
...
#### Scenario: Auto enables when resuming pending tool work
- **GIVEN** reasoning mode is configured as `auto`
- **AND** the session contains at least one pending approval or pending tool batch item
- **WHEN** the agent constructs the next completion request
- **THEN** the agent SHALL request reasoning from the provider
...
- **AND** the session contains no pending approval and no pending tool batch item
