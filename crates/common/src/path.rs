...
impl SandboxedPath {
    ...
    pub fn join<P: AsRef<Path>>(&self, path: P) -> Result<PathBuf> {
        ...
        for component in path.components() {
            match component {
                std::path::Component::ParentDir => return Err(anyhow!("Traversal (..) not allowed in path: {:?}", path)),
                std::path::Component::RootDir | std::path::Component::Prefix(_) => return Err(anyhow!("Absolute components not allowed in path: {:?}", path)),
                _ => {}
            }
        }
