...
pub fn draw(f: &mut Frame, app: &App, area: Rect) {
    ...
    let status_text = match app.state.status {
        ...
        AppStatus::Disconnected => Span::styled("○ Disconnected", Style::default().fg(app.theme.error)),
        AppStatus::Processing => {
            let frame = app.state.spinner_frame % spinner_chars.len();
            Span::styled(format!("{} Processing", spinner_chars[frame]), Style::default().fg(app.theme.highlight))
        }
    };
    ...
    let model_info = Paragraph::new(format!("{} | {}", app.state.active_model, app.state.token_usage))
