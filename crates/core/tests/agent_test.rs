...
use common::llm::{
    CompletionRequest, FunctionCall, LLMProvider, Message, Role, ToolCall
};
...
async fn test_sequential_tool_execution() {
    ...
    config.permissions.overrides.insert("ask_tool".to_string(), PermissionLevel::Ask);
    config.permissions.overrides.insert("allow_tool".to_string(), PermissionLevel::Allow);
    config.permissions.overrides.insert("allow_tool_2".to_string(), PermissionLevel::Allow);

    let mut agent = Agent::new(provider, bus, config, PathBuf::from("."));
    agent.register_tool(Box::new(MockTool { name: "allow_tool".to_string() }));
    agent.register_tool(Box::new(MockTool { name: "ask_tool".to_string() }));
    agent.register_tool(Box::new(MockTool { name: "allow_tool_2".to_string() }));
    ...
    assert!(result.output.as_ref().unwrap().contains("Permission required"));
    ...
    // 2. Approve call_2
    let res = agent.resolve_approval(&mut session, "call_2", true).await.unwrap();
